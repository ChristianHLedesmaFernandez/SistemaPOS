					...
					if(!isNull($telefono)){
						...
						if(!isTelefono($telefono)){
							echo '<script>
								    swal({
									  	type: "error",
									  	title: "¡El telefono puede llevar caracteres especiales!",
					...
					if(!isNull($correo)){			
						if(isEmail($correo)){
							...
							echo '<script>
								    swal({
									  	type: "error",
									  	title: "¡El email puede llevar caracteres especiales!",
									  	showConfirmButton: true,						
									  	confirmButtonText: "Cerrar"
									  	}).then(function(result){
									  		if(result.value){						
					...
					if($local == "agregarlocal"){
						...
						if(isNombre($nombreLocal)){							
							...
							if(!localExiste($nombreLocal)){
								...
								$item = "nombre";
					            $valor =$nombreLocal;
					            $local = ControladorLocales:: ctrMostrarLocales($item, $valor);
				...
				// Si viene el telefono pregunto si es correcto caso contrario continuo	
					...
					if($respuesta){
						...
						if($envio){
							// Envio el mail.
							//
							echo("<script>console.log('Envio el correo');</script>");
							//
							// Obtener id Usuario
							$item = "usuario";
					        $valor =$usuario;
